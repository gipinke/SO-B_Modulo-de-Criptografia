...
static void __exit ebbchar_exit(void)
{
   ...
   kfree(message);
   printk(KERN_INFO "EBBChar: Goodbye from the LKM 1!\n");
   free_crypto();
   printk(KERN_INFO "EBBChar: Goodbye from the LKM 2!\n");
}
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset)
{
   ...
   split_operation(buffer, &op, message, size_of_message);
   ...
   switch(op){
      case 'c': //Cripografia
	sg_init_one(&sk.sg, message, 32);
    	skcipher_request_set_crypt(req, &sk.sg, &sk.sg, 32, ivdata);

	ret = crypto_skcipher_encrypt((&sk)->req);
        pr_info("EBBChar: ret: %d\n", ret);
	printk(KERN_INFO "EBBChar: op: %c, data: %s\n", op, message);

	//sg_init_one(&sk.sg, data, 32);
    	skcipher_request_set_crypt(req, &sk.sg, &sk.sg, 32, ivdata);
